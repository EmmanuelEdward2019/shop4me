
-- Enable PostGIS extension
CREATE EXTENSION IF NOT EXISTS postgis SCHEMA extensions;

-- Function: get_available_orders_nearby
-- Returns pending, unassigned orders whose store is within the agent's service radius.
-- Falls back to zone matching if no GPS coords are available.
-- Agent location comes from agent_applications (service_latitude, service_longitude, service_radius_km).
CREATE OR REPLACE FUNCTION public.get_available_orders_nearby(p_agent_id uuid)
RETURNS SETOF orders
LANGUAGE plpgsql
STABLE
SECURITY DEFINER
SET search_path = public
AS $$
DECLARE
  v_agent_lat numeric;
  v_agent_lng numeric;
  v_radius_km numeric;
  v_agent_zone text;
BEGIN
  -- Get agent's service location from their approved application
  SELECT service_latitude, service_longitude, service_radius_km
  INTO v_agent_lat, v_agent_lng, v_radius_km
  FROM agent_applications
  WHERE user_id = p_agent_id AND status = 'approved'
  LIMIT 1;

  -- Get agent's service zone from profile
  SELECT service_zone INTO v_agent_zone
  FROM profiles
  WHERE user_id = p_agent_id;

  -- If agent has GPS coordinates, use PostGIS distance filtering
  IF v_agent_lat IS NOT NULL AND v_agent_lng IS NOT NULL AND v_radius_km IS NOT NULL THEN
    RETURN QUERY
    SELECT o.*
    FROM orders o
    LEFT JOIN stores s ON lower(trim(s.name)) = lower(trim(o.location_name))
    WHERE o.status = 'pending'
      AND o.agent_id IS NULL
      AND (
        -- Match by store GPS within agent radius
        (s.latitude IS NOT NULL AND s.longitude IS NOT NULL AND
         ST_DWithin(
           ST_MakePoint(s.longitude, s.latitude)::geography,
           ST_MakePoint(v_agent_lng::float8, v_agent_lat::float8)::geography,
           v_radius_km * 1000  -- convert km to meters
         ))
        OR
        -- Fallback: if store has no GPS, match by zone
        (s.latitude IS NULL AND (
          (v_agent_zone IS NOT NULL AND o.service_zone = v_agent_zone)
          OR (v_agent_zone IS NULL AND o.service_zone IS NULL)
        ))
        OR
        -- No matching store found at all, fall back to zone
        (s.id IS NULL AND (
          (v_agent_zone IS NOT NULL AND o.service_zone = v_agent_zone)
          OR (v_agent_zone IS NULL AND o.service_zone IS NULL)
        ))
      )
    ORDER BY o.created_at DESC;
  ELSE
    -- No GPS on agent, fall back to zone-only matching
    RETURN QUERY
    SELECT o.*
    FROM orders o
    WHERE o.status = 'pending'
      AND o.agent_id IS NULL
      AND (
        (v_agent_zone IS NOT NULL AND o.service_zone = v_agent_zone)
        OR (v_agent_zone IS NULL AND o.service_zone IS NULL)
      )
    ORDER BY o.created_at DESC;
  END IF;
END;
$$;
