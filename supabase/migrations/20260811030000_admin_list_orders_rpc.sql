-- Scalability (audit SCALE-1/2): server-side paginated + searchable orders list
-- for the admin dashboard. Replaces "fetch ALL orders + ALL profiles into the
-- browser". Joins buyer/agent/rider names, filters by status + free-text search,
-- and returns a page plus the total row count in one round-trip.
--
-- SECURITY DEFINER + explicit admin guard (the function bypasses RLS, so the
-- guard is what enforces access).
...
-- Helpful indexes for the filter/sort paths.
CREATE INDEX IF NOT EXISTS idx_orders_status_created_at ON public.orders (status, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_orders_location_name_trgm ON public.orders (location_name);
